...
module.exports = {
  ...
  async messageRun(message, args, data) {
    ...
    await message.safeReply("Nenhum comando correspondente encontrado");
  },
  ...
  async interactionRun(interaction) {
    ...
    // No matching command/category found
    await interaction.followUp("Nenhum comando correspondente encontrado");
  },
};
...
async function getHelpMenu({ client, guild }) {
  ...
  for (const [k, v] of Object.entries(CommandCategory)) {
    ...
    options.push({
      label: v.name,
      value: k,
      description: `Ver os comandos na categoria ${v.name}`,
      emoji: v.emoji,
    });
  }

  const menuRow = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder().setCustomId("help-menu").setPlaceholder("Selecione uma categoria").addOptions(options)
  );
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(`Estou aqui para ajudar você!\nDê uma olhada nos comandos disponíveis!`);
...
function getSlashCategoryEmbeds(client, category) {
  ...
  if (category === "IMAGE") {
    ...
    const embed = new EmbedBuilder()
      ...
      .setAuthor({
        name: `${CommandCategory[category]?.name || category} Comandos`,
      })
  ...
  if (commands.length === 0) {
    const embed = new EmbedBuilder()
      ...
      .setDescription("Sem comandos nesta categoria");
  ...
  while (commands.length) {
    ...
    toAdd = toAdd.map((cmd) => {
      ...
      return `\`/${cmd.name}\`\n ❯ **Descrição**: ${cmd.description}\n ${
        !subCmds?.length ? "" : `❯ **Comandos adicionais [${subCmds?.length}]**: ${subCmdsString}\n`
      } `;
    });
  ...
  arrSplitted.forEach((item, index) => {
    const embed = new EmbedBuilder()
      ...
      .setFooter({ text: `pagina ${index + 1} de ${arrSplitted.length}` });
...
function getMsgCategoryEmbeds(client, category, prefix) {
  ...
  arrSplitted.forEach((item, index) => {
    const embed = new EmbedBuilder()
      ...
      .setDescription(item.join("\n"))
      .setFooter({
        text: `pagina ${index + 1} de ${
          arrSplitted.length
        } | Digite ${prefix}help <command> para mais informações sobre um comando específico`,
      });
    arrEmbeds.push(embed);
  });
